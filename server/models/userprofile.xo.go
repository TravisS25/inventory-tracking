...
type UserProfile struct {
	...
	FullName   string `json:"fullName,omitempty" db:"full_name"`     // full_name
...
func (up *UserProfile) Insert(db httputil.XODB) error {
	...
	const sqlstr = `INSERT INTO public.user_profile (` +
		`title_id, email, password, full_name, is_active, date_joined` +
		`) VALUES (` +
		`$1, $2, $3, $4, $5, $6` +
		`) RETURNING id`

	// run query
	XOLog(sqlstr, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined)
	err = db.QueryRow(sqlstr, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined).Scan(&up.ID)
	if err != nil {
		return err
	}
...
func (up *UserProfile) Update(db httputil.XODB) error {
	...
	// run query
	XOLog(sqlstr, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined, up.ID)
	_, err = db.Exec(sqlstr, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined, up.ID)
	return err
}
...
func (up *UserProfile) Upsert(db httputil.XODB) error {
	...
	// sql query
	const sqlstr = `INSERT INTO public.user_profile (` +
		`id, title_id, email, password, full_name, is_active, date_joined` +
		`) VALUES (` +
		`$1, $2, $3, $4, $5, $6, $7` +
		`) ON CONFLICT (id) DO UPDATE SET (` +
		`id, title_id, email, password, full_name, is_active, date_joined` +
		`) = (` +
		`EXCLUDED.id, EXCLUDED.title_id, EXCLUDED.email, EXCLUDED.password, EXCLUDED.full_name, EXCLUDED.is_active, EXCLUDED.date_joined` +
		`)`

	// run query
	XOLog(sqlstr, up.ID, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined)
	_, err = db.Exec(sqlstr, up.ID, up.TitleID, up.Email, up.Password, up.FullName, up.IsActive, up.DateJoined)
	if err != nil {
		return err
	}
